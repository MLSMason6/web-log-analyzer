...
foreach ($lines as $line) {
    ...
    if ($parsed) {
        ...
        // Status codes
        $statusCounts[$parsed['status']] =
            ($statusCounts[$parsed['status']] ?? 0) + 1;

        // IP frequency
        $ipCounts[$parsed['ip']] =
            ($ipCounts[$parsed['ip']] ?? 0) + 1;
...
<form method="POST" action="export_csv.php" style="margin-top:20px;">
    <input type="hidden" name="logs"
        value='<?= json_encode(array_filter($parsedLogs, fn($l) => !empty($l["threats"]))) ?>'>
